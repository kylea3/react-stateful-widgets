...
export default function Programmers() {
  ...
  const [featuredProgrammer, setFeaturedProgrammer] = useState(false);
  ...
  const getNameOfFeatured = () => {
    ...
    return featuredProgrammer;
  };

  const style = {
    fontSize: '1.5em',
    marginTop: '0.5em',
    color: featuredProgrammer !== false ? 'gold' : 'royalblue', // 🤔 color turns to gold, when celebrating
  };
      ...
      <div className='programmers'>
        {
          ...
          programmersList.map(dev =>
            <div className='programmer' key={dev.id}>
              {dev.name} <button onClick={() => {setFeaturedProgrammer(dev.name)}}>Feature</button>
            </div>
          )
        }
      </div>
